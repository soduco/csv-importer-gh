...
const peliasModel = require( 'pelias-model-gh' );
...
function getValidTime(record) {

  const start = getCaseInsensitive('validtime_start', record);
  const end = getCaseInsensitive('validtime_end', record);
...
function processRecord(record, next_uid, stats) {
  ...
  try {
    ...
    const vtime = getValidTime(record);
    if (vtime) {
      pelias_document.setValidTime(vtime);
    }
